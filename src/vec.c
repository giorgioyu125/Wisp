...
int vec_push(Vec **v_ptr, const void *value)
{
    ...
    if (vec->elem_num >= vec->maxcap) {
        ...
        new_vec->bump_ptr =
            (char *) (new_vec + 1) + (new_vec->elem_num * new_vec->elem_size);
...
{
    if (!v_ptr || v_ptr->elem_num == 0 || !out_value)
        return -1;
...
int vec_rem_idx(Vec *v_ptr, size_t idx)
{
    if (!v_ptr)
        return -1;
    if (idx >= v_ptr->elem_num)
        return -2;

    char *data = (char *) (v_ptr + 1) + (idx * v_ptr->elem_size);
    size_t bytes_to_move = (v_ptr->elem_num - idx - 1) * v_ptr->elem_size;

    if (bytes_to_move > 0) {
        memmove(data, data + v_ptr->elem_size, bytes_to_move);
    }
...
{
    if (!v_ptr || !out || idx >= v_ptr->elem_num)
        return -1;
    memcpy(out, (char *) (v_ptr) + sizeof(Vec) + (idx * v_ptr->elem_size),
           v_ptr->elem_size);
    return 0;
}
