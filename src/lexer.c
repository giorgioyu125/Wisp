...
Vec *
lex_tokens (const char *source, size_t source_len)
{
    ...
    while (ptr < end)
	  ...
	  if (c == '(')
	    {
		Token tok = {.type = TOKEN_LPAREN,.value =
	  ...
	  if (c == ')')
	    {
		Token tok = {.type = TOKEN_RPAREN,.value =
			(char *) ptr,.value_len = 1
		};
		vec_push (&tokens, &tok);
		ptr++;
	  ...
	  if (c == '\'')
	    {
		Token tok = {.type = TOKEN_QUOTE,.value =
			(char *) ptr,.value_len = 1
		};
		vec_push (&tokens, &tok);
		ptr++;
	  ...
	  if (c == ',')
	    {
		Token tok = {.type = TOKEN_COMMA,.value =
			(char *) ptr,.value_len = 1
		};
		vec_push (&tokens, &tok);
		ptr++;
	...
	identifier:
	  ...
	  if (isalpha (c) || strchr ("!@#$%^&*-+=<>/?", c))
		...
		Token tok = {.type = TOKEN_IDENTIFIER,
		    .value = (char *) start,
		    .value_len = (size_t) (ptr - start)
		};
		vec_push (&tokens, &tok);
	  ...
	  Token tok = {.type = TOKEN_ERROR,.value = (char *) ptr,.value_len =
		  1 };
	  vec_push (&tokens, &tok);
	  ptr++;
      }
...
Token
vec_get_token (const Vec *v_ptr, size_t idx)
{
...
void
print_token_value (char *value, size_t value_len)
{
