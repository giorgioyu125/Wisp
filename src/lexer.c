...
Vec *lex_tokens(const char *source, size_t source_len) {
  ...
  while (ptr < end) {
    ...
    // LPAREN
    if (c == '(') {
      Token tok = {.type = TOKEN_LPAREN, .value = (char *)ptr, .value_len = 1};
      vec_push(&tokens, &tok);
      ptr++;
      continue;
    }
    ...
    // RPAREN
    if (c == ')') {
      Token tok = {.type = TOKEN_RPAREN, .value = (char *)ptr, .value_len = 1};
      vec_push(&tokens, &tok);
      ptr++;
      continue;
    }
    ...
    // QUOTE OPERATOR
    if (c == '\'') {
      Token tok = {.type = TOKEN_QUOTE, .value = (char *)ptr, .value_len = 1};
      vec_push(&tokens, &tok);
      ptr++;
      continue;
    }
    ...
    // COMMA OPERATOR
    if (c == ',') {
      Token tok = {.type = TOKEN_COMMA, .value = (char *)ptr, .value_len = 1};
      vec_push(&tokens, &tok);
      ptr++;
      continue;
    }
  ...
  identifier:
    ...
    Token tok = {.type = TOKEN_ERROR, .value = (char *)ptr, .value_len = 1};
    vec_push(&tokens, &tok);
    ptr++;
  }
